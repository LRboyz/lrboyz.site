...
export const createTransitionView = (params: TransitionViewParams): FC<PropsWithChildren<BaseTransitionProps>> => {
  ...
  const TransitionView = (props: PropsWithChildren<BaseTransitionProps>) => {
    ...
    return (
      <MotionComponent
        initial={useMemo(() => (lcpOptimization ? initial || from : initial || from), [])}
        animate={{
          ...to,
          transition: {
            duration,
